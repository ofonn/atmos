...
export function BottomNav({ inline = false }: { inline?: boolean }) {
  ...
  return (
    <nav
      className={
        inline
          ? 'relative flex-shrink-0 flex justify-center py-1.5'
        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 px-1 py-1 rounded-full transition-all duration-300 active:scale-90 ${
